...
try:
    ...
    print("Warning: catboost is not installed. To run this script, please run: pip install catboost")
import time
...
# --- 1. XGBOOST ENSEMBLE (5 SEEDS) ---
print("\n=== Training 5-Seed XGBoost Ensemble ===")
...
for seed in seeds:
    ...
    xgb_preds.append(xgb_model.predict(dtest))

mean_xgb_pred = np.mean(xgb_preds, axis=0)

# --- 2. LIGHTGBM ENSEMBLE (5 SEEDS) ---
print("\n=== Training 5-Seed LightGBM Ensemble ===")
...
# --- 3. CATBOOST ENSEMBLE (5 SEEDS) ---
print("\n=== Training 5-Seed CatBoost Ensemble ===")
# For CatBoost, passing high-cardinality string IDs can be slow,
# so we convert them to strings but use Target Encodings as numerical inputs.
# Here we keep operation as a string category.
X_train_cat = X_train.copy()
X_test_cat = X_test.copy()
X_train_cat['operation'] = X_train_cat['operation'].astype(str)
X_test_cat['operation'] = X_test_cat['operation'].astype(str)

# We drop the raw account IDs for CatBoost to keep it fast and prevent RAM issues,
# as the chronological Target Encoding (origin_te, destination_te) already captures all history.
catboost_features = [f for f in features if f not in ["origin_account", "destination_account"]]
X_train_cat = X_train_cat[catboost_features]
X_test_cat = X_test_cat[catboost_features]

cat_features_indices = ["operation"]

cat_preds = []
for seed in seeds:
    print(f"Training CatBoost model with seed {seed}...")
    cb_model = CatBoostClassifier(
        ...
        random_seed=seed,
        verbose=100,
        task_type='CPU' # Change to 'GPU' if running on a GPU-enabled Colab instance!
    )
    cb_model.fit(
        X_train_cat, y_train,
        cat_features=cat_features_indices
    )
    cat_preds.append(cb_model.predict_proba(X_test_cat)[:, 1])

mean_cat_pred = np.mean(cat_preds, axis=0)

# --- 4. FINAL THREE-MODEL BLEND ---
# 80% XGBoost, 10% LightGBM, 10% CatBoost
print("\nBlending predictions (80% XGB, 10% LGB, 10% CatBoost)...")
final_pred = 0.80 * mean_xgb_pred + 0.10 * mean_lgb_pred + 0.10 * mean_cat_pred
...
submission.to_csv("submission_with_catboost.csv", index=False)
print("Submission file successfully created at submission_with_catboost.csv!")
