...
# 🚀 FEATURES V11 (Edge ID, Round amounts)
...
# 🚀 NOUVELLES FEATURES V12 (Dynamic Edge Analysis)
print("Computing V12 Edge Dynamics...")
# 1. Vitesse de l'Arête (Time diff between transfers on exact same edge)
combined['edge_time_diff'] = (combined['period'] - combined.groupby('edge_id')['period'].shift(1)).fillna(999)

# 2. Clone de Transaction (Repeated amount on same edge)
...
# 3. Volume de l'Arête (Cumulative transaction count on edge)
combined['edge_cum_tx_count'] = combined.groupby('edge_id').cumcount()

# 4. Montant cumulé de l'Arête (Cumulative sum of amount on edge)
...
xgb_features = [f for f in features if f != "edge_id"] # drop high cardinality edge_id for XGBoost memory safety
cb_features = [f for f in features if f not in ["origin_account", "destination_account", "edge_id"]] # Avoid RAM issues in CatBoost
