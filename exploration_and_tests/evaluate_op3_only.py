...
# Scenario 1: Train on all data
X_train_all = train_fe[features]
y_train_all = train_fe["fraud_flag"]
X_val_all = val_fe[features]
y_val_all = val_fe["fraud_flag"]

print("\n--- Training on ALL operations ---")
dtrain_all = xgb.DMatrix(X_train_all, label=y_train_all, enable_categorical=True)
dval_all = xgb.DMatrix(X_val_all, label=y_val_all, enable_categorical=True)
...
y_pred_all = model_all.predict(dval_all, iteration_range=(0, model_all.best_iteration + 1))
score_all = average_precision_score(y_val_all, y_pred_all)
print(f"PR-AUC (All operations): {score_all:.6f}")


# Scenario 2: Train ONLY on op_03
print("\n--- Training ONLY on op_03 operations ---")
# Filter train and validation sets
train_op3 = train_fe[train_fe['operation'] == 'op_03']
val_op3 = val_fe[val_fe['operation'] == 'op_03']

X_train_op3 = train_op3[features]
y_train_op3 = train_op3["fraud_flag"]
X_val_op3 = val_op3[features]
y_val_op3 = val_op3["fraud_flag"]

dtrain_op3 = xgb.DMatrix(X_train_op3, label=y_train_op3, enable_categorical=True)
dval_op3 = xgb.DMatrix(X_val_op3, label=y_val_op3, enable_categorical=True)

model_op3 = xgb.train(
    params,
    dtrain_op3,
    num_boost_round=1000,
    evals=[(dtrain_op3, 'train'), (dval_op3, 'val')],
    early_stopping_rounds=50,
    verbose_eval=50
)

# Predict on all validation instances
# We use model_op3 to predict on validation instances, but for non-op_03 we set to 0.0
dval_all_op3 = xgb.DMatrix(X_val_all, enable_categorical=True)
y_pred_op3_raw = model_op3.predict(dval_all_op3, iteration_range=(0, model_op3.best_iteration + 1))

y_pred_op3_final = np.zeros(len(val_fe))
is_op3_val = (val_fe['operation'] == 'op_03').values
y_pred_op3_final[is_op3_val] = y_pred_op3_raw[is_op3_val]

score_op3 = average_precision_score(y_val_all, y_pred_op3_final)
print(f"PR-AUC (Only op_03 model + post-processing): {score_op3:.6f}")
