...
def compute_chronological_te(df, group_col, target_col, smoothing):
...
for smooth in [2, 5, 10, 20, 50]:
    print(f"\nEvaluating smoothing={smooth}...")
    ...
    combined['origin_te'] = compute_chronological_te(combined, 'origin_account', 'fraud_flag', smoothing=smooth)
    combined['destination_te'] = compute_chronological_te(combined, 'destination_account', 'fraud_flag', smoothing=smooth)
    ...
    print(f"Smoothing {smooth} -> PR-AUC: {score:.6f}")
