...
for seed in seeds:
    ...
    lgb_preds.append(lgb_model.predict(X_val, num_iteration=lgb_model.best_iteration))
...
# 3. Evaluate Blends
print("\nEvaluating Blends of 5-Seed XGBoost and 5-Seed LightGBM...")
for w_lgb in [0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3]:
    w_xgb = 1.0 - w_lgb
    y_pred_blend = w_lgb * mean_lgb_pred + w_xgb * mean_xgb_pred
    blend_score = average_precision_score(y_val, y_pred_blend)
    print(f"Weight LGB: {w_lgb:.2f}, Weight XGB: {w_xgb:.2f} -> Blend PR-AUC: {blend_score:.6f}")
