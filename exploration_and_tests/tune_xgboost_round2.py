"""
Description: Second cycle de recherche par grille pour affiner les paramètres d'XGBoost (profondeur, learning rate).
...
# Grid search parameters round 2
configs = [
    {"max_depth": 7, "learning_rate": 0.08, "subsample": 0.8},
    {"max_depth": 6, "learning_rate": 0.10, "subsample": 0.8},
    {"max_depth": 6, "learning_rate": 0.08, "subsample": 0.75},
]

for i, cfg in enumerate(configs):
    print(f"\n--- Running config {i+1}/{len(configs)}: {cfg} ---")
    params = {
    ...
    print(f"Result for config {cfg}: Best Round = {model.best_iteration}, Validation PR-AUC = {val_pr_auc:.6f}")
