...
# ============================================================
# FEATURE ENGINEERING (V8 BASE - Le plus performant)
# ============================================================
...
# Filter confident test predictions (only on op_03 for safety)
idx_0 = (test_preds_s1 < 0.15) & (~is_not_op3_test)
idx_1 = (test_preds_s1 > 0.60) & (~is_not_op3_test)
...
pseudo_w_0 = pd.Series([1.0]*len(pseudo_X_0)) # Strong anchor for safe 0s

pseudo_X_1 = X_test[idx_1].copy()
pseudo_y_1 = pd.Series([1.0]*len(pseudo_X_1))
pseudo_w_1 = pd.Series([0.3]*len(pseudo_X_1)) # Soft weight for risky 1s
...
# Retrain XGB and LGB ONLY with Pseudo-Labels (CatBoost doesn't support instance weights as easily in its API wrapper without Pool, keeping it simple)
oof_xgb_s2 = np.zeros(len(X_train)) # OOF still on original train
oof_lgb_s2 = np.zeros(len(X_train))
test_xgb_s2 = np.zeros(len(X_test))
test_lgb_s2 = np.zeros(len(X_test))

# We use original StratifiedKFold to validate on original train data ONLY
for fold, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
    print(f"\n--- PL Fold {fold + 1} ---")
    
    # Fold Train = original train[fold] + ALL pseudo labels
    ...
    X_val = X_train.iloc[val_idx] # Validate ONLY on actual ground truth
