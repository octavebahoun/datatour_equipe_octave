...
t0 = time.time()
print("=== V11: New Features (Edge ID, Round Amounts) & 3-Model Blend ===")

# ============================================================
# CHARGEMENT DES DONNÉES
# ============================================================
# Détection automatique de l'environnement Kaggle ou local
...
# Assignation des labels d'ensemble
train_raw['is_test'] = 0
test_raw['is_test'] = 1
test_raw['fraud_flag'] = np.nan

# Regroupement des données pour le traitement global
...
def compute_chronological_te(df, group_col, target_col, smoothing=10):
    """
    Calcule le Target Encoding chronologique en décalant les statistiques cumulatives d'un cran (shift(1))
    pour éviter tout transfert d'information futur vers le passé (Data Leakage).
    """
    ...
    # Fusion et application du lissage
...
# Échelle logarithmique pour le montant
combined["amount_log1p"] = np.log1p(np.maximum(combined["amount"], 0))

# Variations absolues des balances
...
# Ratios des montants de transactions sur le solde d'origine et de destination initial
...
# Variables booléennes sur le changement de balance (comptes statiques)
...
# Écarts de temps historiques avec lags de 1, 2, 3 (fréquences de transactions)
for lag in [1, 2, 3]:
...
# Agrégations temporelles par expéditeur
...
# Agrégations temporelles par destinataire
...
# Cardinalité des relations (nombre de relations uniques)
...
# Calcul du Target Encoding chronologique
...
# 🚀 Encodage cible pour la relation unique (arête)
combined['edge_te'] = compute_chronological_te(combined, 'edge_id', 'fraud_flag', smoothing=5)

# Indice spécifique aux opérations op_03
...
# Conversion des chaînes en type catégoriel
...
features = [
    ...
    "edge_id", "is_round_1000", "is_round_5000", "edge_te" # 🚀 NOUVELLES FEATURES
]
cat_features = ["operation", "origin_account", "destination_account", "edge_id"]

# On ignore les catégorielles origin/dest/edge pour CatBoost car elles explosent la RAM et CatBoost est lent avec
cb_features = [f for f in features if f not in ["origin_account", "destination_account", "edge_id"]]

# ============================================================
# STAGE 1: ENTRAÎNEMENT DES MODÈLES & VALIDATION
# ============================================================
print("\n" + "="*50)
...
# Exclusion de edge_id de XGBoost pour éviter les fuites de mémoire
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
    ...
    # 2. LightGBM (avec catégories complètes)
    ...
    # 3. CatBoost (avec exclusion sélective pour la RAM)
...
# ============================================================
# STAGE 2: PONDÉRATION DE L'ENSEMBLE (BLENDING)
# ============================================================
def neg_ap(weights):
    """Calcule l'opposé de la précision moyenne sur les plis OOF pour l'optimisation"""
    w = weights / np.sum(weights)
    blend = w[0] * oof_xgb + w[1] * oof_lgb + w[2] * oof_cb
    return -average_precision_score(y_train, blend)

# Optimisation SLSQP pour trouver les coefficients de blending optimaux
res = minimize(
...
# Calcul des prédictions finales pondérées
final_pred = w1[0]*test_xgb + w1[1]*test_lgb + w1[2]*test_cb

# ============================================================
# POST-PROCESSING
# ============================================================
# Seule l'opération op_03 présente des fraudes. Toutes les autres prédictions sont écrasées à 0.0.
