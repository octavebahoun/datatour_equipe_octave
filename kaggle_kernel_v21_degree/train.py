...
t0 = time.time()
print("=== V21: Base V19 + Global Degrees ===")
...
print("Calcul des caractéristiques de degré globales...")
all_accounts = pd.concat([combined['origin_account'], combined['destination_account']]).unique()
orig_counts = combined['origin_account'].value_counts()
dest_counts = combined['destination_account'].value_counts()

node_degrees = pd.DataFrame(index=all_accounts)
node_degrees['global_out_degree'] = node_degrees.index.map(orig_counts).fillna(0)
node_degrees['global_in_degree'] = node_degrees.index.map(dest_counts).fillna(0)
node_degrees['global_unique_dests'] = node_degrees.index.map(combined.groupby('origin_account')['destination_account'].nunique()).fillna(0)
node_degrees['global_unique_origins'] = node_degrees.index.map(combined.groupby('destination_account')['origin_account'].nunique()).fillna(0)
node_degrees['global_out_dispersion'] = node_degrees['global_unique_dests'] / (node_degrees['global_out_degree'] + eps)
node_degrees['global_in_dispersion'] = node_degrees['global_unique_origins'] / (node_degrees['global_in_degree'] + eps)

node_degrees = node_degrees.reset_index().rename(columns={'index': 'account'})

combined = combined.merge(node_degrees.rename(columns={'account': 'origin_account'}), on='origin_account', how='left')
combined = combined.merge(node_degrees.rename(columns={'account': 'destination_account', 
                                                       'global_out_degree': 'dest_global_out_degree',
                                                       'global_in_degree': 'dest_global_in_degree',
                                                       'global_unique_dests': 'dest_global_unique_dests',
                                                       'global_unique_origins': 'dest_global_unique_origins',
                                                       'global_out_dispersion': 'dest_global_out_dispersion',
                                                       'global_in_dispersion': 'dest_global_in_dispersion'}), on='destination_account', how='left')

# Remplir les NaNs pour les degrés globaux
for col in ['global_out_degree', 'global_in_degree', 'global_unique_dests', 'global_unique_origins', 'global_out_dispersion', 'global_in_dispersion',
            'dest_global_out_degree', 'dest_global_in_degree', 'dest_global_unique_dests', 'dest_global_unique_origins', 'dest_global_out_dispersion', 'dest_global_in_dispersion']:
    combined[col] = combined[col].fillna(0)

del G, edges_weights, pagerank_scores, pagerank_df, node_degrees; gc.collect()
...
features = [
    ...
    "global_out_degree", "global_in_degree", "global_unique_dests", "global_unique_origins", "global_out_dispersion", "global_in_dispersion",
    "dest_global_out_degree", "dest_global_in_degree", "dest_global_unique_dests", "dest_global_unique_origins", "dest_global_out_dispersion", "dest_global_in_dispersion"
]
...
# On injecte les features importantes de niveau 1
stack_key_feats = [
