...
t0 = time.time()
print("=== V14: Account Age + Amount Rank + LightGBM Stacking ===")
...
# 🚀 NOUVELLES FEATURES V14
print("Computing V14 features: Account Age + Amount Rank...")

# 1. Account Age: period - first_seen_period
...
# 2. Amount Rank: amount / max_amount_so_far for this account
combined['orig_max_amount_so_far'] = combined.groupby('origin_account')['amount'].cummax().shift(1)
combined['orig_max_amount_so_far'] = combined.groupby('origin_account')['orig_max_amount_so_far'].fillna(method='ffill').fillna(0)
# Fix: need to use expanding max excluding current row
...
features = [
    ...
    "orig_account_age", "dest_account_age", "orig_amount_rank", "dest_amount_rank" # 🚀 V14
]
...
# ============================================================
# TRAINING (Level-1: 3 base models)
# ============================================================
print("\n" + "="*50)
...
# ============================================================
# BLENDING: Scipy Linear + LightGBM Stacking => pick best
# ============================================================
print("\n" + "="*50)
print("LEVEL-2: SCIPY BLEND vs LGB STACKING")
print("="*50)

# --- Method 1: Scipy Linear Blend (baseline from V12) ---
def neg_ap(weights):
...
print(f"[Scipy]  Weights: XGB={w1[0]:.3f}, LGB={w1[1]:.3f}, CB={w1[2]:.3f} => OOF PR-AUC={scipy_oof_score:.5f}")

# --- Method 2: LightGBM Stacking Meta-Learner ---
stack_train = pd.DataFrame({'xgb': oof_xgb, 'lgb': oof_lgb, 'cb': oof_cb})
stack_test = pd.DataFrame({'xgb': test_xgb, 'lgb': test_lgb, 'cb': test_cb})

# Add a few key features to help the meta-learner
...
skf2 = StratifiedKFold(n_splits=5, shuffle=True, random_state=123)  # Different seed to avoid alignment
...
print(f"[Stack]  LGB Meta-Learner => OOF PR-AUC={stack_oof_score:.5f}")

# --- Pick the best ---
if stack_oof_score > scipy_oof_score:
    print(f"\n✅ STACKING wins! ({stack_oof_score:.5f} > {scipy_oof_score:.5f})")
    final_pred = test_stack
else:
    print(f"\n✅ SCIPY BLEND wins! ({scipy_oof_score:.5f} >= {stack_oof_score:.5f})")
